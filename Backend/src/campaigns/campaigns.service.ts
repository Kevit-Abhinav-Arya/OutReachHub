...
export class CampaignsService {
  ...
  async updateCampaign(
    ...
    const { name, targetedTags, templateId } = updateCampaignDto;
    ...
    if (targetedTags !== undefined) {
      if (!Array.isArray(targetedTags) || targetedTags.length === 0) {
        throw new BadRequestException('Target tags must be a non-empty array');
      }
      updateFields.targetTags = targetedTags.filter((tag) => tag && tag.trim());
    }
    ...
    console.log(updatedCampaign.createdBy);
    return {
  ...
  async copyCampaign(
    ...
    const newName = copyCampaignDto.name || `${originalCampaign.name} (Copy)`;
    ...
    return {
      ...
      campaign: {
        id: copiedCampaign._id.toString(),
        name: copiedCampaign.name,
        targetTags: copiedCampaign.targetTags,
        templateId: copiedCampaign.templateId._id.toString(),
        status: copiedCampaign.status as CampaignStatus,
        targetContactsCount: contactsCount,
