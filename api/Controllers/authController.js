...
const userLogin = async (req, res) => {
  try {
    ...
    if (user.workspaces.length === 0) {
      return res.status(403).json({ message: 'No workspaces assigned to this user' });
    }
    ...
    res.json({
      message: 'Multiple workspaces found. Please select a workspace within 5 minutes',
      tempToken,
      user: {
        id: user._id,
        email: user.email,
...
const selectWorkspace = async (req, res) => {
  try {
    ...
    if (!tempToken || !workspaceId) {
      return res.status(400).json({ message: 'Temporary token and workspaceId are required' });
    }
    ...
    try {
      ...
      if (error.name === 'TokenExpiredError') {
        return res.status(401).json({ message: 'Temporary token expired. Please login again.' });
      }
