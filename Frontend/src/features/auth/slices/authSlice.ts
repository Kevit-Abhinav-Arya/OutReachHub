...
export const selectWorkspace = createAsyncThunk(
  ...
  async (data: SelectWorkspaceRequest, { rejectWithValue }) => {
    try {
      ...
      const message = error.response?.data?.message || 'Workspace selection failed';
      return rejectWithValue(message);
    }
  }
);
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(adminLogin.fulfilled, (state, action: PayloadAction<AdminLoginResponse>) => {
        state.isLoading = false;
    ...
    builder
      ...
      .addCase(selectWorkspace.fulfilled, (state, action: PayloadAction<SelectWorkspaceResponse>) => {
        state.isLoading = false;
        state.user = action.payload.user;
        state.token = action.payload.token;
        state.isAuthenticated = true;
...
// Export actions
export const { clearError, clearWorkspaceSelection, clearAuth } = authSlice.actions;
...
export const selectRequiresWorkspaceSelection = (state: any) => state.auth.requiresWorkspaceSelection;
export const selectAvailableWorkspaces = (state: any) => state.auth.availableWorkspaces;
...
export const selectIsViewer = (state: any) => {
  const user = state.auth.user;
  return user?.type === 'admin' || user?.role === 'Editor' || user?.role === 'Viewer';
};
